...
namespace ad {
    ...
    int naive_detect(string imagefilename){
        ...
        empty_image = cv::Mat::zeros(src.rows, src.cols, CV_8UC1);
    ...
    // Create red channel
        ...
        cv::cvtColor(result_blue,img1,CV_RGB2GRAY);
        cv::cvtColor(result_red,img2,CV_RGB2GRAY);
        cv::cvtColor(result_green,img3,CV_RGB2GRAY);

        //imshow("img1", img1 );

        Mat result = img1-img2-img3;

        //imshow("img1+img2", result );
        ...
        //cout << gray.rows << endl;
        //cv::HoughCircles(gray, circles, CV_HOUGH_GRADIENT, 10, 10, 10, 25);
        ...
        cv::HoughCircles(
                ...
                1,                  // double dp
                10,                 // double minDist
                40,                // double param1=100
                10,                // double param2=100
                1,                  // int minRadius=0
                35                  // int maxRadius=0
        );

        //imwrite(imagefilename + ".points.jpg", im_with_keypoints );

        if(!circles.empty() ){
            cout << "hits:" << circles.size() << endl;
